...
class ProductionService:
    ...
    def create_run(self, payload: ProductionRunCreate, current_user: CurrentUser) -> ProductionRunRead:
        ...
        run = ProductionRun(
            ...
            # El banco de procesos ya no trae limite de merma propio (seccion
            # 3): se usa el mismo default que tenia ProductionProcess antes.
            waste_limit_percent=Decimal("1"),
        ...
        # El banco de procesos (seccion 3) ya no preconfigura insumos por
        # etapa -- se agregan sueltos despues, directo en el acta (mismo
        # mecanismo ADMIN_STOCK/MANUAL de siempre). configured_ingredients
        # queda siempre vacio; stage_ingredients del payload debe venir vacio
        # tambien (lo exige el chequeo de mas abajo).
        ...
        # El banco de procesos ya no tiene sub-etapas (seccion 3): el proceso
        # elegido ES la unica etapa de esta corrida.
        run.stages.append(
            ProductionRunStage(
                ...
                stage_code=_stage_code_for(process.name, run_seq, 1),
            )
        )
        ...
        # insumos, resultantes). Eventos reales la siguen alimentando despues
        # (aprobar materiales, material adicional, merma por etapa,
        # recepcion) — todavia no implementado.
    ...
    def _split_run_for_partial_material(self, run: ProductionRun, covered_qty: Decimal) -> ProductionRun:
        ...
        mismo folio raiz. Reparte el plan de productos y los insumos de etapa
        proporcionalmente a la MISMA fraccion que cubrio la materia prima."""
        ...
        # Las etapas de la hija se derivan de las de la propia orden padre (no
        # se re-consulta el banco de procesos, que ya no tiene sub-etapas): es
        # lo que de verdad esta materializado en esta corrida.
        for stage in sorted(run.stages, key=lambda s: s.stage_order):
            stage_code = _stage_code_for(stage.stage_name, run_seq, stage.stage_order)
            if split_suffix:
                stage_code = f"{stage_code}-{split_suffix}"
            child_ingredients = []
            for ingredient in list(stage.ingredients):
    ...
    def _compute_coverage(self, run: ProductionRun, target_qty: Decimal) -> "_MaterialCoverage":
        ...
        igual materia prima y cada insumo de etapa: la fraccion mas corta
        entre todos manda, y esa MISMA fraccion se aplica a todos al partir
        la orden (ver _split_run_for_partial_material).
        ...
        # resto de recursos (insumos de etapa) se evalua contra esa MISMA
        # fraccion, para que el preview refleje exactamente lo que
    ...
    def reserve_material(
        ...
        # materia prima pero un insumo sigue en cero, la materia prima igual
        # se guarda y la orden espera el resto -- que es justamente el punto
        # de "reservar y esperar". Atarlo al minimo hacia que un recurso en
        # cero impidiera reservar el que si llego.
        from backend.modules.inventory.models import InventoryItem
    ...
    def request_additional_material(
        ...
        insumos de creacion: queda PENDIENTE hasta que Inventario lo aprueba
        (consume stock de verdad) o lo rechaza."""
        run = self.repository.get_run(run_id)
        if run is None:
            raise ProductionNotFoundError("Orden de produccion no encontrada.")
    ...
    def allocate_material(
        ...
        """Inventario destina un ingreso nuevo de materia prima a una corrida
        ESPERANDO_MATERIAL: aprueba materiales e inicia la corrida
        automaticamente. La cobertura real (cuanto alcanza) la calcula
        approve_materials considerando materia prima y cada insumo de etapa
        pendiente, y toma el minimo; esta funcion no debe adelantarse a
        bloquear por su cuenta: approve_materials es quien decide, parte de
        nuevo si hace falta y solo arranca lo que de verdad esta cubierto por
        completo."""
        run = self.repository.get_run(run_id)
        if run is None:
            raise ProductionNotFoundError("Orden de produccion no encontrada.")
    ...
    def start_stage_attempt(
        ...
        code = _stage_attempt_code_for(order_code, process.name, attempt_no) if order_code else None
